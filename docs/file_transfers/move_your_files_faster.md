...
For example, say you have some data sitting on Pylon storage at Xsede. We recommend using a tool like [Globus](/video/research-computing-using-lss-globus) to move your final results back to [LSS](/large-scale-storage-lss) for long-term storage.  When you initiate your transfer, be sure to pick an endpoint like Condodtn (which has a fast network connection to LSS), rather than routing the globus traffic through your desktop connection to LSS.